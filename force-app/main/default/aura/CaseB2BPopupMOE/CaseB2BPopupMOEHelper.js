({
    ...
    handlefileAttached: function(component, event, helper,casid) {
        ...
        try{
            ...
            var fileStudentNameExcel = component.get('v.fileListOfStudentName');
            
            var fEIDvsMedical = component.get('v.mergedListofEIDvsMedical');
            var allFilesData  = component.get('v.mergedAllFilesCompanion');
            var StudentNameActivity  = component.get('v.mergedAllFilesstudentvsActivity');
            ...
            if (StudentNameActivity && StudentNameActivity.length > 0 && type==component.get("v.clActivitiesEvents")) {
                action.setParams({
                    recordId: recordIdString,
                    filesData: JSON.stringify(StudentNameActivity)
                });
                
                
            }
            else if (fileActiVsAwerList && fileActiVsAwerList.length > 0) {
                action.setParams({
                    recordId: recordIdString,
            ...
            }else if(fEIDvsMedical && fEIDvsMedical.length > 0 && type==component.get("v.clHandicapServicesTransportationorNannyRequest")){
                action.setParams({
                    recordId: recordIdString,
                    filesData: JSON.stringify(fEIDvsMedical)
                });
            }else if(allFilesData && allFilesData.length > 0 && type==component.get("v.clRequestaboardingpassforCompanionHandycamptransportation")){
                action.setParams({
                    recordId: recordIdString,
    ...
    handleFileUpload: function(component, event, targetAttribute) {
        try {
            var files = component.get("v.uploadedDocs");
            var fileUploadWrapper = component.get(targetAttribute);
            
            if (files && files.length > 0 && fileUploadWrapper.length < 2) {
                for (var i = 0; i < files[0].length; i++) {
                    var file = files[0][i];
                    
                    if (file.size <= 500000) { // Allow only files less than or equal to 1 MB
                        var reader = new FileReader();
                        reader.name = file.name;
                        reader.type = file.type;
                        
                        reader.onloadend = function(e) {
                            var fileType = e.target.type;
                            
                            if (fileType.includes("image") || fileType.includes("pdf")) {
                                // Process valid files (image or PDF)
                                ...
                                component.set(targetAttribute, fileUploadWrapper);
                                ...
                                this.showToast('Error!', 'Unsupported file type.', 'error');
                            }
                        };
      ...
      removeFileFromList: function (component, listAttributeName, index) {
        var fileList = component.get(listAttributeName);
        if (fileList && index >= 0 && index < fileList.length) {
            fileList.splice(index, 1);
            component.set(listAttributeName, fileList);
        }
    }
